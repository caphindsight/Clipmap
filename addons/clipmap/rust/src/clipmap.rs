...
impl IPrimitiveMesh for ClipmapMesh {
  ...
  fn on_get(&self, name: StringName) -> Option<Variant> {
  ...
  fn on_set(&mut self, name: StringName, value: Variant) -> bool {
    if streq(&name, "clipmap/number_of_lods") {
      if let Ok(val) = value.try_to::<i64>() {
  ...
  fn create_mesh_array(&self) -> AnyArray {
    ...
    if n_lods == 0 {
      ...
      return st.commit_to_arrays().upcast_any_array();
    }
    ...
    st.commit_to_arrays().upcast_any_array()
  }
}
